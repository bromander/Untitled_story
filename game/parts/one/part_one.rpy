...
label to_do_list:
    ...
    menu:
        ...
        "всё равно запилить ролик" if ch_con and not ch_content:
            ...
            $ ch_content = True
        "всё равно лечь спать" if not persistent.is_sleeps:
            jump tuk_tuk_tuk
        "проснуться" if persistent.is_sleeps:
            $ renpy.quit()
...
label tuk_tuk_tuk: #wip
    ...
    menu:
        ...
        "Да":
            ...
            show screen mashenbka_text("Это дурная дрёма... открой глаза, проснись!")
            $ renpy.pause(delay = 3, hard=True)
            show screen mashenbka_text("ПРОСНИСЬ")
            $ renpy.pause(delay = 5, hard=True)
            hide screen mashenbka_text
